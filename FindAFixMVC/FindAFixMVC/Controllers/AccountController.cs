...
namespace FindAFixMVC.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> UserRegister(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    // return RedirectToAction("Account", "ConfirmEmail");
        ...
        [AllowAnonymous]
        public ActionResult TechnicianRegister()
        {
            return View();
        }
        ...
        [HttpGet]
        public ActionResult TechnicianProfileEdit(int id)
        {
            using(var db = new ApplicationDbContext())
            {
                var profile = db.TechnicianProfiles.Find(id);
                return View(profile);
            }
        }
        ...
        private ActionResult RedirectToLocal(string returnUrl, string userName, String password)
        {
            if (Url.IsLocalUrl(returnUrl))
            {
                return Redirect(returnUrl);
            }
            //get user to be signed in through parameters and get the role of the user
            var user = UserManager.FindByEmail(userName);

            //if user is in 'User' role, redirect to the UserHome page, else redirect to the TechnicianHome page
